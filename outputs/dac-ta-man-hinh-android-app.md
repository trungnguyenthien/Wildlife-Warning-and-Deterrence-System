...
> - Tab `[SETTING_TAB]`: cài đặt chung (ngôn ngữ, theme, đăng xuất) + nút điều hướng sang:
...
Mọi thao tác **thêm / xoá** SĐT là transact trực tiếp với server từng hành động — không có nút "Lưu chung" cuối màn.
...
| `remove_recipient_confirm_button` | Button (Danger) | Label `Xoá`. Gọi API xoá + đóng dialog + cập nhật list. |
...
> Lưu ý: validation **chỉ chặn format** & trùng local phía client — server vẫn là nguồn kiểm tra cuối cùng (có thể trả 409 nếu trùng sau khi client đã check, vd thêm cùng lúc trên 2 thiết bị).

### Hành vi

- Mở màn: gọi API lấy danh sách SĐT hiện tại → render `sms_recipient_list_view`. Nếu rỗng → hiện `empty_state_text`.
- Nhấn `add_recipient_button` → mở `add_recipient_dialog` với input trống, focus sẵn vào `add_recipient_phone_input`. Bàn phím số tự mở (`inputType = number`).
- Validate SĐT **realtime** khi user gõ (không chờ bấm Thêm). Trạng thái `add_recipient_save_button` thay đổi theo:
  ...
  - Hợp lệ nhưng trùng → enabled, nhưng hiện lỗi inline, bấm vẫn được và server sẽ trả 409 (an toàn).
...
- Nhấn `add_recipient_save_button` (SĐT hợp lệ):
  ...
  - Khi lỗi 409: hiện lỗi inline trong dialog `SĐT này đã được đăng ký bởi tài khoản khác.`.
...
- Pull-to-refresh → gọi lại API lấy list (đồng bộ khi có thay đổi từ thiết bị khác).
- Trong khi chờ API (loading) → disable `add_recipient_button` + `recipient_remove_iconbutton`, hiển thị spinner nhỏ tại vị trí thao tác.
