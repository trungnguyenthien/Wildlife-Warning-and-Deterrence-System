...
## 4. Quy định đồng bộ cấu hình YAML tĩnh (Hard Configuration)
Khi thực hiện thay đổi, bảo trì hoặc bổ sung các tính năng liên quan đến cấu hình phòng vệ mặc định (`DANGER_PRESETS`) hoặc kịch bản mẫu (`PRESET_SCENARIOS`), bạn BẮT BUỘC phải tuân thủ các quy tắc sau:
1.  **Không chỉnh sửa tĩnh trong mã nguồn:** Tuyệt đối không khai báo hoặc thay đổi trực tiếp cấu hình tĩnh này trong các tệp JavaScript/TypeScript như `configController.ts` hay `eventController.ts`. Tất cả cấu hình này phải được đọc động từ tệp [danger-presets.yaml](file:///Users/trungnguyen/GitHub/Wildlife-Warning-and-Deterrence-System/wildlife-mobile-server/hard-config/danger-presets.yaml).
2.  **Đồng bộ ràng buộc cơ sở dữ liệu và API:** 
    *   **Màu LED (`ledColor`):** Phải là một trong các giá trị chuỗi sau: `RED`, `YELLOW`, `WHITE`, `STROBE` (hoặc `null`).
