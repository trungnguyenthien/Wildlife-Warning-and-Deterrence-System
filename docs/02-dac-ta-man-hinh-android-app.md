...
> ℹ️ **Màn này KHÔNG hiển thị live video streaming.** Nó hiển thị **ảnh snapshot** gần nhất từ camera. Theo thuật toán ở [01-de-tai-nghien-cuu-canh-bao-dong-vat.md:131-170](./01-de-tai-nghien-cuu-canh-bao-dong-vat.md#L131-L170), hệ thống chỉ ghi nhận snapshot mỗi 2 giây/lần khi có chuyển động đáng kể, do đó ảnh trong màn này có thể đã cũ vài giây đến vài phút tuỳ mức độ hoạt động của thú.
...
| `sound_type_dropdown` | Dropdown | Label `Loại âm thanh xua đuổi`. Lấy danh sách ID âm thanh nạp sẵn trong phần cứng từ mảng `animalDeterrentSounds` của API `GET /audio-samples` (ID bắt đầu bằng prefix `A_`, ví dụ mặc định: `Tiếng súng` · `Tiếng gầm` · `Tiếng chó sủa lớn` · `Tiếng nổ giả lập` · `Tiếng người xua đuổi`). |
