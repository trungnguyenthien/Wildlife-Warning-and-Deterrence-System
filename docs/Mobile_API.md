...
```json
{
  ...
  "detections": [
    ...
    { "speciesId": "elephant", "speciesName": "Voi", "confidence": 0.70, "dangerLevel": "CRITICAL" }
  ],
...
### 7.4. `GET /audio-samples`

Lấy danh sách các file âm thanh xua đuổi và các mẫu phát loa thông báo. Mapping: dropdown `sound_type_dropdown` và `speaker_message_dropdown` tại màn hình `[SPECIES_CONFIG_DETAIL_SCREEN]`.

**Response 200**
```json
{
  "items": [
    { "id": "gunshot",     "displayName": "Tiếng súng",     "previewUrl": "...", "type": "SOUND_EFFECT" },
    { "id": "growl",       "displayName": "Tiếng gầm",      "previewUrl": "...", "type": "SOUND_EFFECT" },
    { "id": "dog_bark",    "displayName": "Tiếng chó sủa lớn", "previewUrl": "...", "type": "SOUND_EFFECT" },
    { "id": "explosion",   "displayName": "Tiếng nổ giả lập",  "previewUrl": "...", "type": "SOUND_EFFECT" },
    { "id": "ultrasonic",  "displayName": "Tần số siêu âm",    "previewUrl": "...", "type": "SOUND_EFFECT", "frequencyHz": 22000 },
    { "id": "tpl-01",      "displayName": "Mẫu 1 - Cảnh báo voi hoang dã xuất hiện", "previewUrl": "...", "type": "TTS_TEMPLATE" },
    { "id": "tpl-02",      "displayName": "Mẫu 2 - Phát hiện thú dữ xâm lấn nguy cấp", "previewUrl": "...", "type": "TTS_TEMPLATE" },
    { "id": "tpl-03",      "displayName": "Mẫu 3 - Thông báo di tản & chỉ dẫn lánh nạn", "previewUrl": "...", "type": "TTS_TEMPLATE" }
  ]
}
```
...
### 7.5. `GET /audio-samples/{sampleId}/stream`
...
Khi người dùng từ Mobile App kích hoạt chế độ ghi đè (`override`) hoặc phát âm thanh test thử loa (`test`), Mobile Server sẽ gửi bản tin JSON thông báo qua kết nối WebSocket này:
...
```json
{
  ...
  "payload": {
    "commandId": "cmd-123",
    "deviceKey": "speaker | led_flash | electric_fence",
    "action": "ON | OFF | TEST",
    "params": {
      "volume": 80,
      "audioSampleId": "gunshot"
    }
  }
}
```
...
| 5.4 | GET | `/cameras/stream` | Nhận thông báo cập nhật thời gian thực qua kết nối SSE để tự động reload |
| 6.1 | GET | `/cameras/{cameraId}/devices/state` | Xem chi tiết trạng thái và nguồn kích hoạt của 6 thiết bị |
| 6.2 | POST | `/cameras/{cameraId}/devices/{key}/override` | Điều khiển ghi đè (bật/tắt thủ công nhanh) từng thiết bị |
| 6.3 | POST | `/cameras/{cameraId}/devices/override-all` | Ghi đè bật/tắt đồng loạt toàn bộ thiết bị ngoại vi |
...
| # | Method | Endpoint | Mô tả chức năng |
|---|---|---|---|
| 6.4 | POST | `/cameras/{cameraId}/devices/{key}/test` | Gửi lệnh test thiết bị còi/LED tại hiện trường (`sound_test_button`) |
...
| 7.4 | GET | `/audio-samples` | Tải danh sách file âm thanh xua đuổi & mẫu phát loa (dropdowns) |
| 7.5 | GET | `/audio-samples/{id}/stream` | Stream nghe thử file âm thanh xua đuổi |
...
**Tổng cộng: 51 API di động + 2 API tích hợp thiết bị (gồm 1 Webhook nhận diện và 1 WebSocket điều khiển; đã bỏ các chức năng ngôn ngữ, đổi mật khẩu và khôi phục mật khẩu).**
