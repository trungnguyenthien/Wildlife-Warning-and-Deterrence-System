...
### 10.2. `GET /events/{eventId}/snapshots`
...
### 11.2. `POST /alerts/{alertId}/forward`
...
| 10.2 | GET | `/events/{eventId}/snapshots` | Tải danh sách các snapshot ghi lại trong sự kiện |
...
**Tổng cộng: 39 API di động + 2 API tích hợp thiết bị (gồm 1 Webhook nhận diện và 1 WebSocket điều khiển; đã bỏ các chức năng ngôn ngữ, đổi mật khẩu, khôi phục mật khẩu, stream nghe thử âm thanh, và ghi đè trạng thái thiết bị thủ công).**
