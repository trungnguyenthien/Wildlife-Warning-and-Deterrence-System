...
Khi người dùng từ Mobile App kích hoạt chế độ ghi đè (`override`), tắt khẩn cấp, hoặc phát âm thanh test thử loa (`test`), Mobile Server sẽ gửi chủ động bản tin JSON thông báo qua kết nối WebSocket này đến trạm để thực thi tại chỗ:
...
```json
{
  ...
  "payload": {
    "commandId": "cmd-123",
    "deviceKey": "deterrent_audio | speaker | led_flash | electric_fence", 
...
**Tổng cộng: 50 API di động + 2 API tích hợp thiết bị (gồm 1 Webhook nhận diện và 1 WebSocket điều khiển; đã bỏ các chức năng ngôn ngữ, đổi mật khẩu, khôi phục mật khẩu và stream nghe thử âm thanh).**
