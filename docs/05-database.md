...
*   **Quy trình CRUD:**
    *   **READ:** Đọc khi xác thực đăng nhập ([POST /auth/login](03-mobile_api.md#32-post-authlogin) - [Action 2.1: Login & Register Push Token](04-sequence-diagram.md#21-action-login-register-push-token)), kiểm tra thông tin cá nhân ([GET /users/me](03-mobile_api.md#91-get-usersme) - [Action 3.3.1: Load user profile](04-sequence-diagram.md#331-action-load-user-profile)), xác thực danh tính trước khi đăng xuất ([POST /auth/logout](03-mobile_api.md#33-post-authlogout) - [Action 3.3.2: Logout](04-sequence-diagram.md#332-action-logout)), hoặc khi hệ thống kiểm tra vai trò người dùng (`role`).
    *   **CREATE:** Thêm bản ghi khi đăng ký tài khoản mới ([POST /auth/register](03-mobile_api.md#31-post-authregister) - [Action 1.1: Register a new account](04-sequence-diagram.md#11-action-register-a-new-account)).
...
*   **Quy trình CRUD:**
    *   **READ:** Đọc khi tải danh sách SĐT nhận SMS bổ sung ([GET /users/me/sms-recipients](03-mobile_api.md#121-get-usersmesms-recipients) - [Action 7.1: Load SMS recipients list](04-sequence-diagram.md#71-action-load-sms-recipients-list)) hoặc khi Mobile Server quét danh sách SĐT để gửi tin nhắn SMS cảnh báo động vật hoang dã xâm nhập ([Action 1.1: AI Server sends detection snapshot (AI_SERVER)](04-sequence-diagram.md#11-action-ai-server-sends-detection-snapshot-ai_server)).
    *   **CREATE:** Thêm bản ghi khi đăng ký SĐT nhận cảnh báo mới ([POST /users/me/sms-recipients](03-mobile_api.md#122-post-usersmesms-recipients) - [Action 7.2: Add / Remove SMS recipient](04-sequence-diagram.md#72-action-add-remove-sms-recipient)).
    *   **UPDATE:** Không hỗ trợ cập nhật trực tiếp (người dùng cần xóa đi và thêm lại SĐT mới).
    *   **DELETE:** Xóa bản ghi khi người dùng gỡ bỏ SĐT khỏi danh sách ([DELETE /users/me/sms-recipients/{recipientId}](03-mobile_api.md#123-delete-usersmesms-recipientsrecipientid) - [Action 7.2: Add / Remove SMS recipient](04-sequence-diagram.md#72-action-add-remove-sms-recipient)).
...
*   **Quy trình CRUD:**
    *   **READ:** Đọc khi Mobile Server lấy danh sách Push Token để gửi thông báo đẩy (FCM) thời gian thực tới các thiết bị liên quan khi có báo động ([Action 1.1: AI Server sends detection snapshot (AI_SERVER)](04-sequence-diagram.md#11-action-ai-server-sends-detection-snapshot-ai_server)).
    *   **CREATE:** Thêm bản ghi khi thiết bị đăng ký Token FCM mới sau khi đăng nhập thành công ([POST /devices/push-token](03-mobile_api.md#41-post-devicespush-token) - [Action 2.1: Login & Register Push Token](04-sequence-diagram.md#21-action-login-register-push-token)).
    *   **UPDATE:** Cập nhật thời gian hoặc làm mới Token khi ứng dụng gửi Token FCM cập nhật ([POST /devices/push-token](03-mobile_api.md#41-post-devicespush-token) - [Action 2.1: Login & Register Push Token](04-sequence-diagram.md#21-action-login-register-push-token)).
    *   **DELETE:** Xóa Token tương ứng khi người dùng đăng xuất tài khoản ([DELETE /devices/push-token](03-mobile_api.md#42-delete-devicespush-token) - [Action 3.3.2: Logout](04-sequence-diagram.md#332-action-logout)).
...
*   **Quy trình CRUD:**
    *   **READ:** Đọc khi tải danh sách trạm camera ([GET /cameras](03-mobile_api.md#51-get-cameras) - [Action 3.1.1: Load camera list & initial snapshots](04-sequence-diagram.md#311-action-load-camera-list-initial-snapshots)), xem chi tiết trạm ([GET /cameras/{cameraId}](03-mobile_api.md#52-get-camerascameraid) - [Action 4.1: Load camera details & history logs](04-sequence-diagram.md#41-action-load-camera-details-history-logs)), khi Server truyền phát kết nối SSE cập nhật trạng thái động ([GET /cameras/stream](03-mobile_api.md#54-get-camerasstream) - [Action 3.1.2: Register & Listen for updates via SSE](04-sequence-diagram.md#312-action-register-listen-for-updates-via-sse)), hoặc khi Mobile Server xác thực camera nhận diện ([POST /cameras/{cameraId}/detections](03-mobile_api.md#13a1-post-camerascameraiddetections) - [Action 1.1: AI Server sends detection snapshot (AI_SERVER)](04-sequence-diagram.md#11-action-ai-server-sends-detection-snapshot-ai_server)).
    *   **CREATE:** Không hỗ trợ tạo qua API di động (được quản trị viên chèn trực tiếp qua DB/Admin khi lắp đặt trạm mới).
    *   **UPDATE:** Cập nhật khi kiểm lâm đổi tên hiển thị camera ([PATCH /cameras/{cameraId}](03-mobile_api.md#53-patch-camerascameraid) - [Action 4.2: Update camera name](04-sequence-diagram.md#42-action-update-camera-name)), hoặc khi hệ thống tự động cập nhật trạng thái kết nối ONLINE/OFFLINE của camera thông qua kết nối WebSocket ([WS /ws/cameras](03-mobile_api.md#13a2-ws-wscameras) - [Action 1.1: AI Server sends detection snapshot (AI_SERVER)](04-sequence-diagram.md#11-action-ai-server-sends-detection-snapshot-ai_server)).
...
*   **Quy trình CRUD:**
    *   **READ:** Đọc khi tải danh sách loài ([GET /species](03-mobile_api.md#81-get-species) - [Action 5.1: Load species list & configuration overview](04-sequence-diagram.md#51-action-load-species-list-configuration-overview)), hiển thị dropdown bộ lọc ([Action 3.2.1: Initialize filter & Apply filter (statistics_filter)](04-sequence-diagram.md#321-action-initialize-filter-apply-filter-statistics_filter)), hoặc khi Server quét mức độ nguy cấp để chạy thuật toán Fallback phòng vệ.
...
| `speaker_sample_id`| `VARCHAR(50)` | `NULL` | ID mẫu phát loa cảnh báo người dân (`N_voi_rung`, `N_thu_du`, `N_di_tan`, ...). Được truyền nhận qua API dạng `speakerSampleId`. |
...
| `detected_at` | `TIMESTAMP` | `NOT NULL` | Thời điểm ghi nhận/phát hiện gần nhất của sự kiện |
| `snapshot_url` | `VARCHAR(255)` | `NOT NULL` | Đường dẫn ảnh snapshot chính ghi lại loài thú |


*   **Quy trình CRUD:**
    *   **READ:** Đọc khi tải nhật ký sự kiện lịch sử của trạm ([GET /events](03-mobile_api.md#101-get-events) - [Action 4.1: Load camera details & history logs](04-sequence-diagram.md#41-action-load-camera-details-history-logs)), xem thông tin chi tiết camera ([GET /cameras/{cameraId}](03-mobile_api.md#52-get-camerascameraid) - [Action 4.1: Load camera details & history logs](04-sequence-diagram.md#41-action-load-camera-details-history-logs)), hoặc tính toán thống kê xu hướng/heatmap ([GET /stats/summary](03-mobile_api.md#102-get-statssummary) - [Action 3.2.3: Load trend chart & movement heatmap (per_camera_analysis_section)](04-sequence-diagram.md#323-action-load-trend-chart-movement-heatmap-per_camera_analysis_section)).
    *   **CREATE:** Tạo bản ghi mới khi trạm AI gửi nhận diện lần đầu ([POST /cameras/{cameraId}/detections](03-mobile_api.md#13a1-post-camerascameraiddetections) - [Action 1.1: AI Server sends detection snapshot (AI_SERVER)](04-sequence-diagram.md#11-action-ai-server-sends-detection-snapshot-ai_server)) và trạm này chưa có sự kiện nào đang diễn ra (tức là không có event nào cách thời điểm hiện tại ít hơn 5 phút).
    *   **UPDATE:** Cập nhật khi hệ thống ghi nhận khung hình mới (cập nhật ảnh `snapshot_url`, kéo dài thời gian `detected_at`).
...
*   **Quy trình CRUD:**
    *   **READ:** Đọc khi hiển thị thông tin phân tích loài được nhận diện trong sự kiện (kết hợp tải trong [GET /events](03-mobile_api.md#101-get-events) - [Action 4.1](04-sequence-diagram.md#41-action-load-camera-details-history-logs) và [GET /cameras/{cameraId}](03-mobile_api.md#52-get-camerascameraid) - [Action 4.1](04-sequence-diagram.md#41-action-load-camera-details-history-logs))).
    *   **CREATE:** Thêm bản ghi mới liên tục cho mỗi khung hình nhận dạng được gửi lên từ AI Server trong suốt quá trình sự kiện đang diễn ra ([POST /cameras/{cameraId}/detections](03-mobile_api.md#13a1-post-camerascameraiddetections) - [Action 1.1](04-sequence-diagram.md#11-action-ai-server-sends-detection-snapshot-ai_server)).
...
*   **Quy trình CRUD:**
    *   **READ:** Đọc khi hiển thị luồng cảnh báo tin tức khẩn cấp phân luồng theo vai trò người dùng ([GET /alerts/feed](03-mobile_api.md#111-get-alertsfeed) - [Action 3.2.2: Load weekly detections list (weekly_detections_section)](04-sequence-diagram.md#322-action-load-weekly-detections-list-weekly_detections_section)).
...
*   **Quy trình CRUD:**
    *   **READ:** Đọc khi hiển thị luồng tin tức ([GET /alerts/feed](03-mobile_api.md#111-get-alertsfeed) - [Action 3.2.2: Load weekly detections list (weekly_detections_section)](04-sequence-diagram.md#322-action-load-weekly-detections-list-weekly_detections_section)) để xác định trạng thái `isRead = true` cho tin tức nếu tồn tại bản ghi khớp với `user_id` and `alert_id`.
    *   **CREATE:** Bản ghi được insert tự động khi Mobile Server trả về danh sách tin tức cho người dùng qua API ([GET /alerts/feed](03-mobile_api.md#111-get-alertsfeed) - [Action 3.2.2: Load weekly detections list (weekly_detections_section)](04-sequence-diagram.md#322-action-load-weekly-detections-list-weekly_detections_section)) (hệ thống tự động ghi nhận đã đọc khi user tải danh mục feed).
    *   **UPDATE / DELETE:** Không hỗ trợ.
