...
2.  **Khởi tạo Kế hoạch (Screen Plan):** Tạo tệp kế hoạch thiết kế màn hình dưới định dạng Markdown bên trong thư mục [wildlife-mobile/screen-plans](./screen-plans/) (nếu chưa có thư mục thì tự động tạo mới). Bản kế hoạch bắt buộc phải tuân thủ cấu trúc mẫu sau:
    ...
    *   `## 1. Thành phần Giao diện (UI Components)`: Mô tả chi tiết cách chia các cấu phần UI (chia rõ Screen Composable chính và liệt kê cụ thể các UI Component dùng chung được tái sử dụng từ [wildlife-mobile/UI_COMPONENTS.md](./UI_COMPONENTS.md), cùng với các SubViews nội bộ đặt ở thư mục `components`).
