...
2.  **Khởi tạo Kế hoạch (Screen Plan):** Tạo tệp kế hoạch thiết kế màn hình dưới định dạng Markdown bên trong thư mục [wildlife-mobile/screen-plans](./screen-plans/) (nếu chưa có thư mục thì tự động tạo mới). Bản kế hoạch phải nêu rõ:
    *   Tên màn hình và cách chia các cấu phần UI (chia rõ Screen Composable chính và các SubViews tái sử dụng trong `components`).
    *   Các API sẽ gọi và thời điểm gọi (lúc khởi tạo màn hình, khi click nút, hay realtime stream).
    *   Cấu trúc trạng thái (UI State) và các Action/Event tương tác giữa View và ViewModel.
